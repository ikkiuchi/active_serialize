RSpec.describe ActiveSerialize do
  ...
  describe '#to_h' do
    ...
    context 'when removing some attributes' do
      ...
      it { expect(User.first.to_h.keys).to eq %w[ id name ] }
    end

    context 'when adding some keys' do
      before { User.active_serialize_add :love }
      it { expect(User.first.to_h.keys).to eq %w[ id name created_at updated_at love ] }
      ...
      context 'and a recursive attribute' do
        ...
        it { expect(User.first.to_h.keys).to eq %w[ id name created_at updated_at love books ] }
      ...
      context 'and a named(map) addition key' do
        ...
        it { expect(User.first.to_h.keys).to eq %w[ id name created_at updated_at love method_b ] }
      end
    end
  end

  describe 'Class Method .to_ha' do
    before { User.active_serialize_rmv *%i[ id created_at updated_at ] }
